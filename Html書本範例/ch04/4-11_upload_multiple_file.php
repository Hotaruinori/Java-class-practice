...
for ($i = 0; $i < $fileCount; $i++) {
  if ($_FILES['my_file']['error'][$i] === UPLOAD_ERR_OK){
    print('檔案名稱: ' . $_FILES['my_file']['name'][$i] . '<br>');
    print('檔案類型: ' . $_FILES['my_file']['type'][$i] . '<br>');
    print('檔案大小: ' . ($_FILES['my_file']['size'][$i] / 1024 / 1024) . ' MB<br>');
    print('暫存名稱: ' . $_FILES['my_file']['tmp_name'][$i] . '<br>');
    ...
    if (file_exists('upload/' . $_FILES['my_file']['name'][$i])){
      ...
      $file = $_FILES['my_file']['tmp_name'][$i];  // 取得伺服器端檔案暫存
  ...
  elseif ($_FILES['my_file']['error'][$i] === UPLOAD_ERR_INI_SIZE){
